...
app.config['UPLOAD_FOLDER'] = r'C:\Users\triam\Downloads'
    ...
    if request.method == 'POST':
        ...
        for _, row in df.iterrows():
            ...
            if result[0] == 0:
                ...
                cur.execute("UPDATE grades SET test_score = %s, total_marks = %s WHERE student_id = %s AND faculty_id = %s AND course_id = %s and test_name = %s",
                            (test_score, total_marks, student_id, faculty_id, course_id,test_name,))
...
def final_grades(course_id):
    ...
    if request.method == 'POST':  # Editing the grades
        ...
        # Update the grade in the database for the given student_id, course_id, and faculty_id
        cur = mysql.connection.cursor()
        cur.execute("UPDATE final_grades SET final_grade = %s WHERE student_id = %s AND course_id = %s AND faculty_id = %s",
                    (grade, student_id, course_id, faculty_id))
        mysql.connection.commit()
        cur.close()
        ...
        cur = mysql.connection.cursor()
        cur.execute("SELECT student_id, test_name, test_score, total_marks FROM grades WHERE course_id = %s and faculty_id = %s",
                    (course_id, faculty_id))
        grades = cur.fetchall()
        cur.close()

        # Calculate the weighted average and final grade for each student
        final_grades = {}
        for grade in grades:
            student_id = grade[0]
            test_name = grade[1]
            ...
            if result:
                ...
                if student_id not in final_grades:
                    final_grades[student_id] = {
                        'total_weighted_percentage': weighted_percentage,
                        'total_weight': weight
                    }
                else:
                    final_grades[student_id]['total_weighted_percentage'] += weighted_percentage
                    final_grades[student_id]['total_weight'] += weight

        # Assign the final grade for each student based on the weighted average
        for student_id, grades_info in final_grades.items():
            weighted_avg = grades_info['total_weighted_percentage'] / grades_info['total_weight']
            ...
            final_grades[student_id]['final_grade'] = final_grade

        # Update the existing records or insert new records into the final_grades table
        cur = mysql.connection.cursor()
        for student_id, grades_info in final_grades.items():
            final_grade = grades_info['final_grade']

            cur.execute(
                "INSERT INTO final_grades (student_id, course_id, faculty_id, final_grade) VALUES (%s, %s, %s, %s) "
                "ON DUPLICATE KEY UPDATE final_grade = VALUES(final_grade)",
                (student_id, course_id, faculty_id, final_grade))
        ...
        cur = mysql.connection.cursor()
        cur.execute("SELECT student_id, final_grade FROM final_grades WHERE course_id = %s AND faculty_id = %s",
                    (course_id, faculty_id))
...
def view_distribution(course_id):
    ...
    pie_chart_file = 'fd_s2_6_grades_completed_ig/fd_s2_4/static/pie_chart.png'
...
@app.route('/export_pdf/<course_id>', methods=['GET', 'POST'])
def export_pdf(course_id):
    ...
    if request.method == 'POST':
        ...
        cur = mysql.connection.cursor()
        cur.execute(
            "SELECT date_, hour_, status_ FROM attendance WHERE course_id = %s AND student_id = %s AND faculty_id = %s",
            (course_id, student_id, faculty_id,))
        attendance_records = cur.fetchall()
        cur.close()

        # Generate the PDF file
        filename = f"attendance_report_{course_id}_{student_id}.pdf"
        document_title = f"Attendance Report - Course ID: {course_id}, Student ID: {student_id}"
        ...
        # Write the attendance records to the PDF
        y = 750  # Initial y-coordinate for content
        for record in attendance_records:
            date_ = record[0]
            hour_ = record[1]
            status_ = record[2]
            ...
            c.drawString(50, y, f"Date: {date_}")
            c.drawString(150, y, f"Hour: {hour_}")
            c.drawString(250, y, f"Status: {status_}")
        ...
        # Reset the buffer position and send the PDF as a downloadable attachment
        pdf_data.seek(0)
    ...
    download_name=filename
        ...
        # Retrieve the list of students for the course
        cur = mysql.connection.cursor()
        cur.execute("SELECT student_id FROM student_course WHERE course_id = %s and faculty_id=%s", (course_id,faculty_id,))
        students = [student[0] for student in cur.fetchall()]
        cur.close()

        # Render the template with the dropdown menu and attendance records
        return render_template('exportpdf.html', course_id=course_id, students=students)
